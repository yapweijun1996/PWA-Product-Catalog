...
**Current implementation:** Commit `d5930eb` contains the Vite/React/TypeScript source, Dexie persistence, CSV/JSON import, supplier-shaped fixtures, quote pricing/snapshots, backup/restore, five-locale settings, PDF export/share fallback, manifest/icons, owned Service Worker, browser tests, and Pages workflow. The Pages deployment and update path are verified at https://yapweijun1996.github.io/PWA-Product-Catalog/. Real supplier validation and several release proofs remain open.
...
Status is split between implemented code, local verification, deployment evidence, and release gaps. Local evidence below was run on 2026-09-11 against commit `d5930eb`; deployment evidence was collected from the production Pages URL.
...
| FR-PDF-03..05 | Partially verified | Browser tests prove multi-page output, valid PDF bytes, all five locale export paths, and draft preservation after an injected PDF download setup failure; a deployed synthetic branded PDF was visually inspected for logo, company/customer, notes, totals, and wrapping. Exact renderer-failure behavior, CJK glyph correctness, final branding assets, and representative acceptance PDF remain open. |
...
| FR-DEPLOY-01..03 | Implemented; verified | `vite.config.ts`, `.github/workflows/deploy-pages.yml`, and GitHub Actions run 10 (`34600730823`) passed install, typecheck, 16 unit/integration tests, 13 browser tests, build, artifact upload, and deployment for commit `d5930eb`; runs 11 (`34601321451`) and 12 (`34602091645`) redeployed synchronized documentation commits. |
